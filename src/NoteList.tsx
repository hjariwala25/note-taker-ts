...
export function NoteList({ availableTags, notes, onUpdateTag, onDeleteTag }: NoteListProps) {
    ...
    return <>
        <Row className="aligh-item-center mb-4">
            <Col>
                <h1>Notes</h1>
            </Col>
            <Col xs="auto">
        ...
        <EditTagModel onUpdateTag={onUpdateTag} onDeleteTag={onDeleteTag} show={editTagsModalIsOpen} handleClose={() => setEditTagsModalIsOpen(false)} availableTags={availableTags} />
    </>
}

function NoteCard({ id, title, tags }: SimplifiedNote) {
    return <Card as={Link} to={`/${id}`} className={`h-100v text-reset text-decoration-none ${styles.card}`}>
        <Card.Body>
